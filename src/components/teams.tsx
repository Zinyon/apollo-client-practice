...
function Teams() {
  ...
  function deleteTeamCompleted(data: any){
    console.log(data.deleteTeam);
    alert(`${data.deleteTeam.id} 항목 삭제`);
    refetchTeams();
    setContentId(0);
  }
  ...
  function editTeamCompleted (data: any) {
  ...
  function postTeamCompleted (data: any) {
  ...
  function AsideItems() {
    ...
    return (
      <ul>
        {data.teams.map(({id, manager, members}: {id: number, manager: string, members: [Member]}) => {
          return (
            <li key={id}>
