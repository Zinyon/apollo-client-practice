...
let refetchPeople: any;
let refetchPerson: any;
...
function People() {
  ...
  function postPersonCompleted (data: any) {
  ...
  function editPersonCompleted (data: any) {
  ...
  function deletePersonCompleted (data: any) {
  ...
  function AsideItems () {
    ...
    function peopleFaces(sex: string, id: number) {
      const bySex : {[key: string]: string[]}= {
        male: ['🧑🏿', '👨🏻', '👦🏼', '‍🧓🏽', '🧔🏾'],
        female: ['👩🏻', '👧🏼', '👩🏽‍🦰', '👩🏾‍🦱', '👱🏿‍♀️']
      }
    ...
    return (
      <ul>
        {data.people.map(
          ({id, sex, first_name, last_name, blood_type} : Person) => {
            return (
  ...
  function MainContents () {
    ...
    return (
      <div className="inputContainer">
        ...
        {contentId > 0 && (
          <ul>
            {data.person.tools.map((tool: any) => {
              return (
                <li key={tool.id}>
                  {tool.id}
